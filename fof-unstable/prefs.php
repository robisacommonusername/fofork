...
THIS FILE IS RIDDLED WITH SECURITY ERRORS.  SOME ARE VERY SERIOUS
SEVERAL PERSISTENT XSS, AND AN ACCOUNT HIJACKING ARE POSSIBLE
...
if(isset($_POST['tagfeed']))
{
    ...
    {
        fof_tag_feed(fof_current_user(), $feed_id, $tag); #Persistent XSS
...
if(isset($_GET['untagfeed']))
{
    ...
    $tags = $_GET['tag'];
    $title = $_GET['title'];
	#check intent - I can delete a user's tags by making them click a link
    foreach(explode(" ", $tags) as $tag)
    {
        fof_untag_feed(fof_current_user(), $feed_id, $tag);
        $message .= " Dropped $tag from '$title'."; #XSS
    }
}
...
if(isset($_POST['prefs']))
{
	...
	$prefs->set('order', $_POST['order'] == "desc" ? "desc" : "asc");
	$prefs->set('sharing', $_POST['sharing']); #these need checking
...
if(isset($_POST['plugins']))
{
    ...
    {
        $key = $plugin_pref[1];   
        $prefs->set($key, $_POST[$key]); #no checking at all
    }
...
# You mean I can change anyone's password?? Cool!
if(isset($_POST['changepassword'])) 
{
    ...
    {
        $username = $_POST['username']; #MUST check that user matches logged in user
        $password = $_POST['password'];
        fof_db_change_password($username, $password);
        
        $message = "Changed password for $username."; #And an XSS, just for fun
    }
}
...
if(fof_is_admin() && isset($_POST['adduser']) && $_POST['username'] && $_POST['password']) 
{
    ...
    $password = $_POST['password'];
    #validation on the username, plz

	fof_db_add_user($username, $password);
	$message = "User '$username' added."; #XSS
}
...
<br><font color="red"><?php echo $message # Security - so much XSS here?></font><br>
...
<select name=username><?php echo $delete_options #XSS USER NAMES UNCHECKED?></select>
