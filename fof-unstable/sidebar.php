...
<center id="welcome">Welcome <b><?
#SECURITY - tbc, is there a potential XSS here due to username??
php echo $fof_user_name ?></b>! <a href="prefs.php">prefs</a> | <a href="logout.php">log out</a> | <a href="http://feedonfeeds.com/">about</a></center>
<br>
...
$feeds = fof_get_feeds(fof_current_user(), $order, $direction); #Security - has order been checked? NO
# GAME OVER - ARBITRARY CODE EXECUTION ON SERVER
...
<input id="searchfield" name="search" value="<?php echo $search?>"> <?php #SECURITY - XSS?>
<?php
	if($what == "unread")
		echo "<input type='hidden' name='what' value='all'>";
	else
		echo "<input type='hidden' name='what' value='$what'>"; #SECURITY - XSS
?>
<?php if(isset($_GET['when'])) echo "<input type='hidden' name='what' value='${_GET['when']}'>"  #SECURITY - XSS?>
</form>
</li>
</ul>
...
{   
   ...
   print "<td>";
   if($unread) print "<a class='unread' href='.?what=$tag_name+unread'>$unread</a>/"; #XSS in tag name
   print "<a href='.?what=$tag_name'>$count</a></td>"; #XSS
