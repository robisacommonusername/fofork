...
function fof_get_feeds($user_id, $order = 'feed_title', $direction = 'asc')
{
   ...
   while($row = fof_db_get_row($result))
   {
	...
	#GAME OVER - Arbitrary code execution vuln
   $feeds = fof_multi_sort($feeds, $order, $direction != "asc"); #SECURITY - has $order been checked??
...
function fof_view_title($feed=NULL, $what="new", $when=NULL, $start=NULL, $limit=NULL, $search=NULL)
{
    ...
    {
    	#SECURITY - checked for SQL injection, safe - $feed is escaped
...
function fof_get_nav_links($feed=NULL, $what="new", $when=NULL, $start=NULL, $limit=NULL)
{
    ...
    if(!is_null($when) && $when != "")
    {
        ...
        {
            $whendate = $when; #bug here if when is crap data - need validation
        }
    ...
    if(is_numeric($start))
    {
        ...
        #inputs are unchecked - this will lead to XSS
...
#CHECK THIS FUNCTION CAREFULLY - something feels amiss, and it's a long complex function
function fof_update_feed($id)
{
    ...
    if($admin_prefs['purge'] != "")
    {
        ...
        if(count($ids) != 0)
        {
            ...
            $result = fof_db_query($sql); #SECURITY - SQL INJECTION, perhaps not, may just be bad variable naming
            ...
            {
                $in = implode(", ", $delete); 
                fof_db_query( "delete from $FOF_ITEM_TABLE where item_id in ($in)" ); # Possible sql injection, but prob ok.  $delete is a list of item_ids
...
{
	#creating new functions from passed data is dangerous...
	#GAME OVER - ARBITRARY CODE EXECUTION
    if($rev)
    {
