...
function fof_db_get_items($user_id=1, $feed=NULL, $what="unread", $when=NULL, $start=NULL, $limit=NULL, $order="desc", $search=NULL)
{
    ...
    if(is_numeric($start))
    {
        ...
        $limit_clause = " limit $start, $limit "; #bug - limit clause may not be initialised. Could also contain SQL
    }
    ...
    {
    	#probably safe, since we're using %d which automatically takes care of any SQL in $feed
    ...
    if(!is_null($search) && $search != "")
    {
        ...
        $args[] = $search; #gets escaped in fof_safe_query
        $args[] = $search;
    }
